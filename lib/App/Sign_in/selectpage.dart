...
class selectpage extends StatefulHookWidget {
  @override
...
class _selectpageState extends State<selectpage> {
  ...
  Widget build(BuildContext context) {
    final netStat = useProvider(netProvider);
    return Scaffold(
        appBar: AppBar(
          centerTitle: true,
          title: Text(
        ...
        body: netStat.when(
          data: (res) => body(context, res),
          loading: () => Center(child: CircularProgressIndicator()),
          error: (error, stackTrace) => Container(),
        ));
  }

  body(BuildContext context, ConnectivityResult res) {
    return Container(
      width: 550,
                ...
                onTap: () async {
                  ...
                  if (authenticated) {
                    Navigator.of(context).pushReplacement(
                      MaterialPageRoute(
                        builder: (_) =>
                            isFirstime || ConnectivityResult.none == res
                                ? Selectmonth()
                                : Loginpage(),
                      ),
                    );
